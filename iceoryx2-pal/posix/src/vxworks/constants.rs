...
pub const O_SYNC: int = 0x2000; // from vxsdk/sysroot/usr/h/public/sys/fcntlcom.h
...
pub const MAP_FAILED: *mut void = libc::MAP_FAILED as *mut void;
...
pub const SIGEV_SIGNAL: int = 0x1;
...
pub const SCM_CREDENTIALS: int = 2; // NOTE: not available
...
pub const INADDR_ANY: in_addr_t = libc::INADDR_ANY as _;
...
pub const MSG_NOSIGNAL: int = 0; // not available on VxWorks since it does not raise a SIGPIPE when using send/sento on a closed FD
...
pub const CLOCK_TIMER_ABSTIME: int = libc::TIMER_ABSTIME as _;
