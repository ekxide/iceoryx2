...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --dry-run              Show what benchmarks would be run without executing them"
            ...
            echo ""
            echo "This script runs benchmarks remotely on the specified target using remote-test-client"
            echo ""
            echo "Benchmark packages and their arguments:"
            echo "  benchmark-publish-subscribe: --bench-all --iterations 100000"
            echo "  benchmark-event: --bench-all --iterations 1000"
            echo "  benchmark-request-response: --iterations 100000"
            echo "  benchmark-queue: --iterations 100000"
            exit 0
            ;;
        *)
...
if [ "$DRY_RUN" = true ]; then
    echo -e "${YELLOW}DRY RUN MODE - No benchmarks will be executed${NC}"
    echo ""
fi
...
echo -e "${BLUE}Building benchmarks for specified packages...${NC}"

# Build each benchmark package individually
for package in "${!BENCHMARK_PACKAGES[@]}"; do
    echo -e "${YELLOW}Building $package...${NC}"
    if ! cargo +${TOOLCHAIN} build --target $TARGET --package $package --bins --release; then
        echo -e "${RED}ERROR: Failed to build benchmarks for $package${NC}"
        exit 1
    fi
done
...
# Gather benchmark executables for the specific packages
declare -a BENCHMARK_EXECUTABLES
declare -a BENCHMARK_ARGS

for package in "${!BENCHMARK_PACKAGES[@]}"; do
    # Get executables for this specific package
    mapfile -t PACKAGE_EXECUTABLES < <(cargo +${TOOLCHAIN} build --target $TARGET --package $package --bins --release --message-format=json 2>/dev/null | \
    jq -r --arg pkg "$package" 'select(.reason == "compiler-artifact" and .target.kind[] == "bin" and (.target.name | startswith($pkg))) | .executable')
    
    for executable in "${PACKAGE_EXECUTABLES[@]}"; do
        BENCHMARK_EXECUTABLES+=("$executable")
        BENCHMARK_ARGS+=("${BENCHMARK_PACKAGES[$package]}")
    done
done
...
echo -e "${YELLOW}Found ${#BENCHMARK_EXECUTABLES[@]} benchmark executables${NC}"

if [ "$DRY_RUN" = true ]; then
    echo ""
    echo -e "${BLUE}Benchmarks that would be executed:${NC}"
    for i in "${!BENCHMARK_EXECUTABLES[@]}"; do
        ((CURRENT++))
        BENCHMARK_NAME=$(basename "${BENCHMARK_EXECUTABLES[$i]}")
        echo -e "  ${BLUE}[$CURRENT]${NC} $BENCHMARK_NAME ${BENCHMARK_ARGS[$i]}"
    done
    
    echo ""
    echo -e "${YELLOW}Dry run completed. $CURRENT benchmarks would be executed.${NC}"
    exit 0
fi

set +e  # Don't exit on any error

# Loop all discovered benchmark executables
for i in "${!BENCHMARK_EXECUTABLES[@]}"; do
    ((CURRENT++))
    benchmark_executable="${BENCHMARK_EXECUTABLES[$i]}"
    benchmark_args="${BENCHMARK_ARGS[$i]}"
    BENCHMARK_NAME=$(basename "$benchmark_executable")
    
    echo ""
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}[$CURRENT/${#BENCHMARK_EXECUTABLES[@]}] Running: $BENCHMARK_NAME${NC}"
    echo -e "${BLUE}Arguments: $benchmark_args${NC}"
    echo -e "${BLUE}========================================${NC}"
    
    # Run the benchmark and capture output
    BENCHMARK_START_TIME=$(date +%s)
    BENCHMARK_OUTPUT=$(remote-test-client run 0 "$benchmark_executable" $benchmark_args 2>&1)
    EXIT_CODE=$?
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        echo -e "${GREEN}✓ COMPLETED: $BENCHMARK_NAME (${BENCHMARK_DURATION}s)${NC}"
        ((PASSED++))
    else
        echo -e "${RED}✗ FAILED: $BENCHMARK_NAME (${BENCHMARK_DURATION}s, exit code: $EXIT_CODE)${NC}"
        ((FAILED++))
        FAILED_BENCHMARKS+=("$BENCHMARK_NAME")
    fi

    echo ""
    PROGRESS_PERCENT=$((CURRENT * 100 / ${#BENCHMARK_EXECUTABLES[@]}))
...
echo -e "Target: $TARGET"
echo -e "Toolchain: $TOOLCHAIN"
echo -e "Total benchmarks: $CURRENT"
