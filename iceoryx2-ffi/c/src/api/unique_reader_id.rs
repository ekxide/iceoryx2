...
use iceoryx2::port::port_identifiers::UniqueReaderId;
...
#[repr(C)]
#[iceoryx2_ffi(UniqueReaderId)]
pub struct iox2_unique_reader_id_t {
    pub value: iox2_unique_reader_id_storage_t,
    pub(super) deleter: fn(*mut iox2_unique_reader_id_t),
}

impl iox2_unique_reader_id_t {
    pub(super) fn init(
        &mut self,
        value: UniqueReaderId,
        deleter: fn(*mut iox2_unique_reader_id_t),
    ) {
...
impl AssertNonNullHandle for iox2_unique_reader_id_h_ref {
    fn assert_non_null(self) {
        debug_assert!(!self.is_null());
        unsafe {
            debug_assert!(!(*self).is_null());
...
impl HandleToType for iox2_unique_reader_id_h {
    type Target = *mut iox2_unique_reader_id_t;
...
impl HandleToType for iox2_unique_reader_id_h_ref {
    type Target = *mut iox2_unique_reader_id_t;
