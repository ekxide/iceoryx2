...
#[repr(C)]
#[iceoryx2_ffi(UniqueServerId)]
pub struct iox2_unique_server_id_t {
    pub value: iox2_unique_server_id_storage_t,
    pub(super) deleter: fn(*mut iox2_unique_server_id_t),
}

impl iox2_unique_server_id_t {
    pub(super) fn init(
        &mut self,
        value: UniqueServerId,
        deleter: fn(*mut iox2_unique_server_id_t),
    ) {
...
pub struct iox2_unique_server_id_h_t;
/// The owning handle for [`iox2_unique_server_id_t`]. Passing the handle to an function transfers the ownership.
pub type iox2_unique_server_id_h = *mut iox2_unique_server_id_h_t;
/// The non-owning handle for [`iox2_unique_server_id_t`]. Passing the handle to an function does not transfers the ownership.
pub type iox2_unique_server_id_h_ref = *const iox2_unique_server_id_h;

impl AssertNonNullHandle for iox2_unique_server_id_h {
    fn assert_non_null(self) {
        debug_assert!(!self.is_null());
    }
}

impl AssertNonNullHandle for iox2_unique_server_id_h_ref {
    fn assert_non_null(self) {
        debug_assert!(!self.is_null());
        unsafe {
            debug_assert!(!(*self).is_null());
...
impl HandleToType for iox2_unique_server_id_h {
    type Target = *mut iox2_unique_server_id_t;
...
impl HandleToType for iox2_unique_server_id_h_ref {
    type Target = *mut iox2_unique_server_id_t;
...
#[unsafe(no_mangle)]
unsafe extern "C" fn iox2_unique_server_id_value(
    handle: iox2_unique_server_id_h,
    id_ptr: *mut u8,
    id_length: usize,
) {
    unsafe {
        ...
        if let Some(Some(id)) =
            (h.value.internal.as_ptr() as *const Option<UniqueServerId>).as_ref()
        {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_unique_server_id_drop(handle: iox2_unique_server_id_h) {
    unsafe {
        handle.assert_non_null();
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_unique_server_id_eq(
    lhs: iox2_unique_server_id_h_ref,
    rhs: iox2_unique_server_id_h_ref,
) -> bool {
    unsafe {
        lhs.assert_non_null();
        rhs.assert_non_null();
...
/// * `rhs` - Must be a valid [`iox2_unique_server_id_h_ref`]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_unique_server_id_less(
    lhs: iox2_unique_server_id_h_ref,
    rhs: iox2_unique_server_id_h_ref,
) -> bool {
    unsafe {
